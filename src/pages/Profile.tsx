import { Settings, Edit3, Globe, Bed, Plane, Utensils, ArrowRight, MapPin, Calendar, Star, ShieldCheck, Sparkles, Trash2, Plus } from 'lucide-react';
...
export default function Profile() {
  ...
  const handleCreateTrip = async () => {
    ...
    // Navigate to planner (would use react-router in real implementation)
    console.log('Created new trip:', newTrip);
  };

  const travelStats = {
    trips: userTrips.length,
    miles: userTrips.reduce((sum, trip) => sum + (trip.budget * 0.1), 0), // Rough calculation
    ...
    <main className="pt-24 md:pt-28 pb-16 md:pb-20 px-6 md:px-8 max-w-screen-2xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <aside className="md:col-span-5 lg:col-span-4 space-y-6 md:space-y-8">
            ...
            <div className="relative z-10">
              ...
              <div className="flex justify-center gap-2 md:gap-3 mb-8 md:mb-10">
                ...
                {isEditing ? (
                  ...
                  <button className="px-4 md:px-6 py-2 bg-primary text-white rounded-full text-[10px] md:text-xs font-bold hover:shadow-lg hover:shadow-primary/20 transition-all flex items-center gap-2">
          ...
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-10 text-white shadow-2xl relative overflow-hidden">
            <div className="absolute top-[-20%] right-[-20%] w-64 h-64 bg-primary/20 blur-[80px] rounded-full"></div>
            <h3 className="font-headline text-lg md:text-xl font-bold mb-6 md:mb-8 flex items-center gap-2">
            ...
            <div className="space-y-4 md:space-y-6 relative z-10">
              <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium">Global Entry</span>
                </div>
                <span className="text-[10px] font-bold text-cyan-400 uppercase tracking-widest">Active</span>
              </div>
              <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/10">
                <div className="flex items-center gap-3">
                  <Plane className="text-cyan-400 w-5 h-5" />
                  <span className="text-sm font-medium">Priority Pass</span>
                </div>
                <span className="text-[10px] font-bold text-cyan-400 uppercase tracking-widest">Elite</span>
              </div>
              <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/10">
                <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex justify-between items-end mb-6 md:mb-8">
              ...
              <div className="flex gap-3">
                ...
                <button className="text-primary font-bold text-xs md:text-sm hover:underline flex items-center gap-1">View All <ArrowRight className="w-4 h-4" /></button>
              </div>
            </div>
                ...
                {userTrips.map((trip) => (
                  <div key={trip.id} className="group relative h-72 md:h-80 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-lg">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent p-8 md:p-10 flex flex-col justify-end">
                      ...
                      <div className="flex gap-3">
                        <button className="w-fit px-5 md:px-6 py-2 bg-white/20 backdrop-blur-md border border-white/30 rounded-full text-white text-[10px] md:text-xs font-bold hover:bg-white hover:text-primary transition-all">
                          View Itinerary
                        </button>
                        <button 
                          onClick={() => handleDeleteTrip(trip.id)}
                          className="w-fit px-3 py-2 bg-red-500/80 backdrop-blur-md border border-red-500/30 rounded-full text-white text-[10px] font-bold hover:bg-red-600 transition-all"
                        >
                          Delete
                        </button>
                      </div>
